...
- LMAX Disuptor 比`ArrayBlockingQueue`和`LinkedBlockingQueue`更快。
- [Mechanical sympathy](https://mechanical-sympathy.blogspot.com/)让你成为更好的开发人员。(懂得底层硬件)
- 在线程间共享内存很容易出问题，你需要小心对待。
...
此外，当发生修改必须对另外的线程可见，这叫`修改可见性`

这是并发的两个主要概念。

### 互相排斥

`锁`是实现排斥的一种解决方案，但是他们在公平性上需要仲裁所以显得非常昂贵。操作系统在线程等待锁直到释放之前将其挂起，从而形成上下文
切换。
...
像我们看到的那样，由于内存屏障的介入，结果已经非常接近我们的预期了，然而，即使内存屏障也不足以阻止
非原子操作的条件竞争。
...
而且，传统的队列系统，生产者操作队头的同时消费者操作队尾。如果队列为空，很可能队头，队尾，以及队列的容量都在
同一个`缓存行`中，这将导致上面说的伪共享。
...
> Disruptor 显著的降低了写竞争，更低的并发负载，更加缓存友好，实现了高吞吐量，低延迟。
...
在内部， ring buffer支持对象数组，据创造者说这是一个很好的选择。在硬件层级，数组有一个可预知的访问模式，元素可以预加载，所以事件处理器不用频繁的回到主存中加载下一个元素。这样我们就可以高效地遍历，而不像队列
那样通过链表的方式访问。
...
多生产者的情况下，他们都会收到每个事件，如果我们想分开加载，我们可以实现基于序列号的分片策略，像这样:
...
当我们创建Disruptor实例的时候，我们同时传入了一个`producerType`变量(`ProducerType.SINGLE`或`ProducerType.MULTI`其中之一)，告诉Disruptor我们是单个生产者还是多个生产者。

在单个生产者的情况下，`ringBuffer.next()`方法完全是无锁的，如果是多个生产者，这个方法通过`CAS`操作从ring buffer中获取下一个可用的序列号。
